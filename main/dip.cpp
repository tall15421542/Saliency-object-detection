...
/* part of grabcut*/
void find_largest_connect_region();	//find largest connected region as initial object
void find_connect_pixel(int **used, int x, int y, int now, int *count);	//find connect pixels
...
int main(int argc, char *argv[])
{
	...
	fp = NULL;
	if((fp = fopen("./segmentation/output/target.csv", "r")) == NULL)	fail("open target.csv fail\n");
...
/* set largest connected regions to be initial object */
void find_largest_connect_region()
{
	int **used;
	int i, j;
	int now = 1;
	...
	for(i = 0; i < row; i++){
		for(j = 0; j < column; j++){
			if(!init_trimap[i][j] || used[i][j])	continue;
			int count = 0;
			find_connect_pixel(used, i, j, now, &count);
			if(count > maxcount){
				maxcount = count;
				maxnow = now;
			}
	...
	for(i = 0; i < row; i++){
		for(j = 0; j < column; j++){
			if(used[i][j] == maxnow)	init_trimap[i][j] = 1;
...
void find_connect_pixel(int **used, int x, int y, int now, int *count)
{
	if(x < 0 || x >= row || y < 0 || y >=column)	return ;
	if(used[x][y])	return ;
	if(!init_trimap[x][y])	return ;

	(*count)++;
	used[x][y] = now;
	find_connect_pixel(used, x + 1, y, now, count);
	find_connect_pixel(used, x - 1, y, now, count);
	find_connect_pixel(used, x, y + 1, now, count);
	find_connect_pixel(used, x, y - 1, now, count);
}
